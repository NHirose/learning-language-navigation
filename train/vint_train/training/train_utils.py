...
def train_lelan_col2(
    ...
    smooth_loss_logger = Logger("smooth loss", "train", window_size=print_log_freq)    
    col_loss_logger = Logger("col loss", "train", window_size=print_log_freq)   
    lelan_loss_logger = Logger("lelan loss", "train", window_size=print_log_freq)           
    loggers = {
        "total loss": total_loss_logger,    
        "pose loss": pose_loss_logger,
        "vel smooth loss": smooth_loss_logger,
        "col loss": col_loss_logger,        
        "lelan loss": lelan_loss_logger,
    }
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            (
            ...
            with torch.no_grad():
                obsgoal_cond_noob = model_copy("vision_encoder", obs_img=batch_obs_images, feat_text = feat_text.to(dtype=torch.float32), current_img=batch_obs_current)
                linear_vel_noob, angular_vel_noob = model_copy("dist_pred_net", obsgoal_cond=obsgoal_cond_noob)
            
                px_ref_noob_list, pz_ref_noob_list, ry_ref_noob_list = robot_pos_model_fix(linear_vel_noob, angular_vel_noob)
                
                px_ref_noob_listx = []
                pz_ref_noob_listx = []
                for it in range(8):
                    px_ref_noob_listx.append(px_ref_noob_list[it].unsqueeze(1).unsqueeze(2))
                    pz_ref_noob_listx.append(pz_ref_noob_list[it].unsqueeze(1).unsqueeze(2))
                traj_policy_noob = torch.concat((torch.concat(pz_ref_noob_listx, axis=1), -torch.concat(px_ref_noob_listx, axis=1)), axis=2)
                                
            dist_loss = nn.functional.mse_loss(last_poses, batch_goal_pos)   
            diff_loss = nn.functional.mse_loss(linear_vel[:,:-1], linear_vel[:,1:]) + nn.functional.mse_loss(angular_vel[:,:-1], angular_vel[:,1:]) 
            
            mask_nomad = (batch_goal_pos[:,1:2] > 1.0).float().unsqueeze(1).repeat(1,8,2)
            mask_nomad_inv = (~(batch_goal_pos[:,1:2] > 1.0)).float().unsqueeze(1).repeat(1,8,2)
            mask_dist = (~(batch_goal_pos[:,1:2] > 1.0)).float()
            sum_dist = mask_dist.sum()           
            col_loss = nn.functional.mse_loss(mask_nomad*traj_policy, 0.12*mask_nomad*select_traj)*float(B)/(float(B) - sum_dist.float() + 1e-7) #0.12 is de-normalization
            lelan_loss = nn.functional.mse_loss(mask_nomad_inv*traj_policy, mask_nomad_inv*traj_policy_noob)*float(B)/(sum_dist.float() + 1e-7)
            
            loss = 1.0*dist_loss + 1.0*diff_loss + weight_col_loss*(col_loss + lelan_loss)
            ...
            wandb.log({"vel smooth loss": diff_loss.item()})
            wandb.log({"col loss": col_loss.item()})
            wandb.log({"lelan loss": lelan_loss.item()})
            ...
            if i % print_log_freq == 0:
                ...
                losses['vel smooth loss'] = diff_loss.item()                 
                losses['col loss'] = col_loss.item()       
                losses['lelan loss'] = lelan_loss.item()  
...
def evaluate_lelan_col2(
    ...
    smooth_loss_logger = Logger("smooth loss", eval_type, window_size=print_log_freq)    
    col_loss_logger = Logger("col loss", eval_type, window_size=print_log_freq) 
    lelan_loss_logger = Logger("lelan loss", eval_type, window_size=print_log_freq)     
    loggers = {
        "total loss": total_loss_logger,    
        "pose loss": pose_loss_logger,
        "vel smooth loss": smooth_loss_logger,
    ...
    with tqdm.tqdm(
        ...
        for i, data in enumerate(tepoch):
            ...
            wandb.log({"diff_eval_loss": diff_loss.item()})
            wandb.log({"col_eval_loss": col_loss.item()})
            wandb.log({"lelan_eval_loss": lelan_loss.item()})
                        
            all_total += (dist_loss + 1.0*diff_loss + weight_col_loss*col_loss).item()
            all_dist += dist_loss.item()
            all_diff += diff_loss.item()
            all_col += col_loss.item()  
            all_lelan += lelan_loss.item()                       
            count_batch += 1.0
            data_size += B
            if i % print_log_freq == 0 and print_log_freq != 0:
                losses = {}
    ...
    print(eval_type, "total loss:", all_total/count_batch, "dist loss:", all_dist/count_batch, "diff loss:", all_diff/count_batch, "col loss:", all_col/count_batch, "lelan loss:", all_lelan/count_batch, "batch count:", count_batch, "data size:", data_size)
